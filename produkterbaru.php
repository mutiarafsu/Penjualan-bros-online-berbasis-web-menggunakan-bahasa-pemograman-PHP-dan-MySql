  ...
  <?php
	...
	while($rp = mysql_fetch_array($sqlp)){
	  ...
	  echo "$rp[detailproduk]<p>";
	  echo "<a href='?idk=$_GET[idk]&p=produkdetail&idp=$rp[idproduk]'><button type='button' class='btn btn-more'>Lihat</button></a>";
	  echo " <a href='?p=keranjang&idp=$rp[idproduk]&ida=$ra[idanggota]'><button type='button' class='btn btn-more'>Beli Sekarang</button></a>";
	  echo " <a href='?p=orderpesanan&idp=$rp[idproduk]&ida=$ra[idanggota]'><button type='button' class='btn btn-more'>Pesan</button></a>";
	  echo "</div>";
	}
  ?>
  </div>
